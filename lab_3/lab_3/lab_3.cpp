...
void SendPartMatrixA(int* coords, int send_count, int recv_count, double* A, double* A_part, MPI_Comm column_comm, MPI_Comm row_comm)
{
...
void SendPartMatrixB(int* coords, int B_x, int B_y, int B_part_x, int B_part_y, double* B, double* B_part, MPI_Comm column_comm, MPI_Comm row_comm)
{
...
int main(int argc, char* argv[])
{
    ...
    SendPartMatrixA(coords, A_part_x * A_part_y, A_x * A_y, A, A_part, column_comm, row_comm);
    SendPartMatrixB(coords, B_x, B_y, B_part_x, B_part_y, B, B_part, column_comm, row_comm);

    //Multiply matrices
    MultMatrix(A_part_x, A_part_y, B_part_x, A_part, B_part, C_part);

    //C matrix assembly
    MatrAssembly(size, B_x, A_y, B_part_x, A_part_y, C_part, C, comm2d);
    end = MPI_Wtime();

    if (rank == 0)
    {
        std::cout << "+===================+" << std::endl;
        std::cout << "| Misstakes: " << CheckAnsw(A_x, A_y, B_x, A, B, C) << "      |" << std::endl;
        std::cout << "+===================+" << std::endl;
